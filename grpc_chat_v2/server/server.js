...
const sendMsg = (call, callback) => {
    ...
    observers.forEach((observer) => {
      console.log(`Sending message to: ${observer.call.request.user.name}`)
...
const recieveMsg = (call, callback) => {
  console.log(`Sending message to: ${call.request.user.name} => ${call.request.active ? "ONLINE" : "OFFLINE"}`)
  observers.push({
    call,
  });
};
